...
function Btn({ children, onClick, grad, style = {}, disabled, sm }) {
  return (
    <button onClick={onClick} disabled={disabled} className="cta"
      style={{
        background: disabled ? "rgba(255,255,255,0.1)" : grad || G.pink,
...
function BackBtn({ onClick, label = "Voltar" }) {
  return (
    <button onClick={onClick} style={{
      position: "fixed", top: 16, left: 16, zIndex: 999, display: "flex", alignItems: "center", gap: 7,
    ...
    <div style={{ display: "flex", gap: 4, background: "rgba(0,0,0,0.3)", borderRadius: 999, padding: "3px" }}>
      {opts.map(o => (
        <button key={o.id} onClick={() => setLang(o.id)}
          style={{
            padding: "4px 10px", borderRadius: 999, border: "none", cursor: "pointer", fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 11,
...
const IsoNameInput = React.memo(function IsoNameInput({ init, accentC, onCommit }) {
  ...
  return (
    <input ref={ref} defaultValue={init} placeholder="Como quer ser chamado?" maxLength={24} autoComplete="off" spellCheck={false}
      ...
      style={{
        width: "100%", background: "rgba(255,255,255,0.07)", border: "2px solid rgba(255,255,255,0.12)", borderRadius: 14,
  ...
  return (
    <div>
      ...
      <input type="range" min={6} max={17} step={1} value={age} onChange={e => { const v = parseInt(e.target.value); setAge(v); onCommit(v); }}
        style={{
          width: "100%", height: 6, borderRadius: 99, appearance: "none", outline: "none", cursor: "pointer",
          ...
          {USER_TYPES.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => onSelect(t.id)} onMouseEnter={() => setHov(t.id)} onMouseLeave={() => setHov(null)}
                style={{
                  display: "flex", alignItems: "center", gap: 20, padding: "22px 26px", borderRadius: 22, cursor: "pointer", transition: "all .22s", textAlign: "left", fontFamily: "'Nunito',sans-serif",
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
      {items.map(it => (
        <button key={it.id} onClick={() => onSel(it.id)}
          style={{
            width: 34, height: 34, borderRadius: "50%", background: it.c, cursor: "pointer", flexShrink: 0, border: `3px solid ${selId === it.id ? accentC : "transparent"}`,
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
      {items.map(it => (
        <button key={it.id} onClick={() => onSel(it.id)}
          style={{
            padding: "6px 14px", borderRadius: 999, fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 12, cursor: "pointer",
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {!img ? (
          <div onClick={() => fileRef.current?.click()}
            style={{
              border: `2px dashed ${pInfo.c}66`, borderRadius: 20, padding: "40px 30px",
            ...
            <div style={{ display: "flex", gap: 4, background: "rgba(0,0,0,0.3)", borderRadius: 12, padding: 4 }}>
              {[{ id: "summary", icon: "📋", label: "Resumo" }, { id: "exercises", icon: "✏️", label: "Exercícios" }].map(t => (
                <button key={t.id} onClick={() => setTab(t.id)}
                  style={{
                    flex: 1, padding: "9px 12px", borderRadius: 9, border: "none", cursor: "pointer", fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 13,
...
function downloadPDF(title, content) {
  ...
  doc.save(
    `${title || "MentalGame"}.pdf`
  );

}
async function downloadBeautifulPDF(
  elementId,
  fileName
){
...
/* ══════════════════════════════════════════════════════
   CHAT MODE
══════════════════════════════════════════════════════ */
function ChatMode({ topic, grade, pInfo, lang, isFree, profile, loadHistory }) {
  const gObj = GRADES.find(x => x.id === grade);
  ...
  const send = async () => {
    ...
    try {
      ...
      console.log("SALVANDO CHAT...");
      console.log("UID:", profile?.uid);
      console.log("PERGUNTA:", input);
      console.log("RESPOSTA:", rep);
      console.log("PROFILE:", profile);
      console.log("RESPOSTA IA:", rep);
      await saveHistory(
  profile.uid,
  "chat",
  input,
      ...
      <div style={{ padding: "7px 18px", background: `linear-gradient(90deg,${pInfo.c}22,transparent)`, borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0, flexWrap: "wrap" }}>
        ...
        {isFree && (
          <button onClick={() => setShowImgPanel(true)}
            style={{
              display: "flex", alignItems: "center", gap: 6, padding: "5px 13px", borderRadius: 999, cursor: "pointer", fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 12,
  ...
  <div
    key={i}
    style={{
      display:"flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", gap: 10, alignItems: "flex-end", animation: "fadeUp .2s ease" }}>
...
!m.content.includes("Olá! 👋")
    ...
    (
      ...
      >
        ...
        <button
          onClick={() => printContent(m.content)}
        >
          🖨️ Imprimir
        </button>
    ...
    try {
      ...
      const rep = await callAI(
        [
          {
            role: "user",
            content: `Crie um resumo MUITO didático sobre "${currentTopic
              }" para alunos do ${gObj?.full
              }.
  ...
  return (
    <div style={{ padding: "22px 26px", overflowY: "auto", height: "100%" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
        {sorted.map((st, i) => {
          const pw = getPowerInfo(st.xp || 0); const lvl = getLevel(st.xp || 0); const pwr = getPower(st.xp || 0); const gp = GP[st.grade] || GP.ef6; const gr = GRADES.find(g => g.id === st.grade) || GRADES[6]; return (
...
/* ══════════════════════════════════════════════════════
   MAIN APP  (student)
══════════════════════════════════════════════════════ */
const MODES = [{ id: "chat", label: "Tutor", icon: "🤖" }, { id: "quiz", label: "Quiz", icon: "🎯" }, { id: "summary", label: "Resumo", icon: "📝" }, { id: "exercise", label: "Exercícios", icon: "✏️" }, { id: "speech", label: "Pronúncia", icon: "🎙️" }, { id: "history", label: "Histórico", icon: "📜" }, { id: "challenge", label: "Desafio", icon: "⚔️" }, { id: "game", label: "Mini-jogo", icon: "🕹️" }, { id: "free", label: "Livre", icon: "💬" }];
...
function HistoryMode({ historyData, profile, clearHistory }) {
console.log(
  "HISTORY MODE:",
  historyData
);
...
<button
  onClick={() => clearHistory(profile.uid)}
  style={{
    background:"#ef4444",
    color:"white",
    border:"none",
...
>
  ...
  <h3>
  {item.data?.title || item.title || "Sem título"}
</h3>

<button
  onClick={() =>
    downloadBeautifulPDF(
      `history-content-${item.id}`,
      `${item.title}.pdf`
    )
  }
  style={{
...
function MainApp({ profile, historyData, clearHistory, loadHistory, onProfileUpdate, onEditAvatar, onChangeGrade, onLogout, showGuardianNow }) {
  const isMobile = useIsMobile();
  ...
  const selMode = async (m) => {
    ...
    if (map[m]) {
      ...
      handleDailyTask(map[m]);

    }

    if (profile?.uid && m === "history") {
console.log("LOAD HISTORY:", loadHistory);
  await loadHistory(profile.uid);
...
/* ══════════════════════════════════════════════════════
   ROOT APP
══════════════════════════════════════════════════════ */
async function saveHistory(userId, type, title, data = {}) {
...
transform:translateY(0px) translateX(0px) scale(1);
  ...
  const onUserType = (t) => {
    setUserType(t); if (t === "teacher") { go("teacher"); return; } if (t === "guardian") { const p = ls.get(SK_P); if (p?.grade && p?.name) { setProfile(p); setScreen("main"); } else go("grade-select"); return; } go("grade-select");
  ...
  return (
    <MainApp
      ...
      clearHistory={clearHistory}
      onEditAvatar={onEditAv}
      onChangeGrade={() => go("grade-select")}
      onLogout={() => {
        setScreen("user-type");
        setHistory([]);
      }}
      showGuardianNow={userType === "guardian"}
    />
